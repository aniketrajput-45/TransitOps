    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          {filteredNavigation.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 ${isActive ? "text-indigo-600" : "text-slate-400 group-hover:text-slate-900"}`} />
                {item.name}
              </Link>
            );
          })}
        ...
        {user && (
          <div className="p-4 border-t border-slate-200 bg-slate-50">
            ...
            <button
              ...
              className="w-full flex items-center justify-center px-4 py-2.5 text-xs font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-xl transition gap-2"
            >
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden bg-slate-950/40 backdrop-blur-sm">
          <div className="relative flex flex-col w-64 max-w-xs bg-white border-r border-slate-200 animate-slide-in">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 flex items-center justify-between px-6 bg-white border-b border-slate-200">
          <button
            type="button"
            className="md:hidden p-2 -ml-2 rounded-md text-slate-650 hover:text-slate-900 focus:outline-none"
          ...
          <div className="flex items-center gap-4">
            <span className="hidden sm:inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200">
              {user?.role}
            </span>
          </div>
        </header>
